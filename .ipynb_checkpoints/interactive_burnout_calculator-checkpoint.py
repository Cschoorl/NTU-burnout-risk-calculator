...
def prepare_features(df: pd.DataFrame):
    ...
    bp = df_processed["Blood Pressure"].astype(str).replace("None", "0/0").fillna("0/0")
    bp_split = bp.str.split("/", expand=True)
    df_processed["Systolic BP"] = pd.to_numeric(bp_split[0], errors="coerce").fillna(0)
    df_processed["Diastolic BP"] = pd.to_numeric(bp_split[1], errors="coerce").fillna(0)
    ...
    df_processed["Sleep Disorder"] = (
        df_processed["Sleep Disorder"].astype(str).replace("", "Unknown").fillna("Unknown")
    )
...
def _predict_burnout(person_data: dict | pd.Series):
    ...
    person[["Systolic BP", "Diastolic BP"]] = person["Blood Pressure"].str.split("/", expand=True).astype(float)
    ...
    person["Gender"] = _state["le_gender"].transform(person["Gender"])
    person["BMI Category"] = _state["le_bmi"].transform(person["BMI Category"])
    person["Sleep Disorder"] = _state["le_sleep"].transform(person["Sleep Disorder"])
